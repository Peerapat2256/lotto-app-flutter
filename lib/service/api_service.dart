...
class ApiService {
  ...
  // ---------------- Internal helper ----------------
  Future<Map<String, String>> _buildHeaders({bool withAuth = false, String? token}) async {
    final actualToken = token ?? (withAuth ? (await storage.read(key: 'accessToken') ?? '') : '');
    return {
      'Content-Type': 'application/json',
  ...
  // ---------------- Base HTTP methods ----------------
  Future<dynamic> getRequest(String endPoint, {bool withAuth = false, String? token}) async {
  ...
  Future<dynamic> postRequest(String endPoint, Map<String, dynamic> data,
      {bool withAuth = false, String? token}) async {
    final headers = await _buildHeaders(withAuth: withAuth, token: token);
  ...
  Future<dynamic> patchRequest(String endPoint, Map<String, dynamic> data,
      {bool withAuth = false, String? token}) async {
    final headers = await _buildHeaders(withAuth: withAuth, token: token);
  ...
  Future<Map<String, dynamic>> loginUser(Map<String, dynamic> data) async {
    ...
    if (res is Map<String, dynamic> && res['status'] == 'success') {
      if (res['data'] != null) {
        ...
        await storage.write(key: 'wallet', value: res['data']['wallet'].toString());
        await storage.write(key: 'username', value: res['data']['username']);
        await storage.write(key: 'user_id', value: res['data']['user_id'].toString());
      }
      return res;
    }
  ...
  Future<Map<String, dynamic>> refreshToken() async {
    ...
    if (refreshToken.isEmpty) throw Exception("No refreshToken found in storage");
    return await postRequest('/user/refreshtoken', {'refreshToken': refreshToken});
  }

  Future<Map<String, dynamic>> logout() async {
    final refreshToken = await storage.read(key: 'refreshToken') ?? '';
    final res = await postRequest('/user/logout', {'refreshToken': refreshToken}, withAuth: true);
    await storage.deleteAll();
    return res;
  }

  Future<Map<String, dynamic>> getUserProfile(String email,
      {bool withAuth = true, String? token}) async {
    final encoded = Uri.encodeComponent(email);
    final res = await getRequest('/user/$encoded', withAuth: withAuth, token: token);
    if (res is Map<String, dynamic>) return res;
    throw Exception("Invalid response format for getUserProfile: $res");
  }

Future<List<Map<String, dynamic>>> getLottoList({bool withAuth = true, String? token}) async {
  ...
  Future<Map<String, dynamic>> createPurchase(int userId, int lottoId,
      {bool withAuth = true, String? token}) async {
  ...
  Future<Map<String, dynamic>> cancelPurchase(int purchaseId,
      {bool withAuth = true, String? token}) async {
  ...
  Future<Map<String, dynamic>> checkout(int userId,
      {bool withAuth = true, String? token}) async {
    final res = await postRequest(
      '/api/checkout',
      {"user_id": userId},
  ...
  Future<List<Map<String, dynamic>>> getUserPurchases({bool withAuth = true, String? token}) async {
    ...
    final res = await getRequest('/api/purchases/$userId', withAuth: withAuth, token: token);

      print("API getUserPurchases Response = $res");

    if (res is Map<String, dynamic> && res['success'] == true && res['purchases'] != null) {
  ...
  Future<Map<String, dynamic>> claimPrize(int lottoId, {bool withAuth = true, String? token}) async {
  final res = await postRequest('/api/claim-prize',{"lotto_id": lottoId},withAuth: withAuth,token: token,
  );
  if (res is Map<String, dynamic>) return res;
